...
namespace VrSharp.PvrTexture
{
    public class PvrTexture : VrTexture
    {
        ...
        {
            // GBIX and PVRT
            if (IsValidGbix(source, offset, length))
            {
                return true;
            }

            // PVRT (and no GBIX chunk)
            if (IsValidPvrt(source, offset, length))
            {
                return true;
            }

            // GBIX and PVRT with RLE compression
            if (IsValidGbix(source, offset + 0x04, BitConverter.ToInt32(source, offset + 0x00)))
            {
                return true;
            }

            // PVRT (and no GBIX chunk) with RLE compression 
            if (IsValidPvrt(source, offset + 0x04, BitConverter.ToInt32(source, offset + 0x00)))
            {
                return true;
            }
